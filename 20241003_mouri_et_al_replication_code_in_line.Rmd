...
Allelic_bias_scatter_plot <- ggplot(data=double_mpra_scatter, aes(x=tcell_LogSkew, y=jurkat_LogSkew))+
  ...
  geom_point(data=subset(double_mpra_scatter, jurkat_skew=="emVar"),color="#E14A4A",size=2) + 
  geom_point(data=subset(double_mpra_scatter, tcell_skew=="emVar"),color="#5D93CD",size=2) + 
  geom_point(data=subset(double_mpra_scatter, tcell_skew=="emVar" & jurkat_skew=="emVar"),color="#6da934",size=2) + 
...
#mpra.dir <- "/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/GM12878/glm/data/20240310_GM12878_glm_mpra_merge_hg38.txt"
#mpra_merge <- read.delim(paste0(mpra.dir), header=T, stringsAsFactors = F, sep="\t")
# An input in every function is going to be a subtitle which appears on all of the graphs to indicate which analysis you are preforming. This subtitle is a description in quotes and here we make it an object so we can put it in all of the plots. There will also be inputs for the plots directory and the plots identifier. 
#subtitle <- "GM12878 glm 03/10/24 Primary Tcell filter"
# plots.dir is the place where your plots are stored 
#plots.dir <- "/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/GM12878/glm/plots"
#data.dir <- "/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/GM12878/glm/data"
#plots_id is the prefix to the plot. It should end with an underscore to look nice
#plots_id <- "20240310_GM12878_glm_hg38" 
#cell_type <- "GM12878"
# What is the stimulation status of the cells? We going to eliminate stimulated (S) or unstimulated (U) cells.
#eliminate_stim_status <- "U" # Stimulated so eliminate unstimulated cells with U
```
...
write.table(dat.plot, paste0(data.dir,"/",plots_id,"_encode_dhs_enrichment_table.txt"), row.names=F, col.names=T, sep="\t", quote=F)
...
#mpra.dir <- "/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/GM12878/glm/data/20240310_GM12878_glm_mpra_merge_hg19.txt"
#mpra_merge <- read.delim(paste0(mpra.dir), header=T, stringsAsFactors = F, sep="\t")
# An input in every function is going to be a subtitle which appears on all of the graphs to indicate which analysis you are preforming. This subtitle is a description in quotes and here we make it an object so we can put it in all of the plots. There will also be inputs for the plots directory and the plots identifier. 
#subtitle <- "GM12878 glm 03/10/24 Primary Tcell filter"
# plots.dir is the place where your plots are stored 
...
#data.dir <- "/nfs/jray/screens/ALL_MPRAs/Ho_et_al_analysis/Downstream_Analysis/20240310_mpra_analysis/GM12878/glm/data"
# plots_id is the prefix to the plot. It should end with an underscore to look nice
#plots_id <- "20240310_GM12878_glm_hg19" 
#cell_type <- "GM12878"
# What is the stimulation status of the cells? We going to eliminate stimulated (S) or unstimulated (U) cells.
#eliminate_stim_status <- "U" # Stimulated so eliminate unstimulated cells with U
...
# For each mpra variant, find the disease with the strongest association and its associated PICS data
  ...
  mpra.pics.plot$top_PP_running<-NA #Cumulative sum of posterior probabilities for that variant
...
# Sensitivity and specificity is not precision and recall!
# https://towardsdatascience.com/should-i-look-at-precision-recall-or-specificity-sensitivity-3946158aace1

```

```{r}
 # WIP presentation version
dat.enrichment2 <- subset(dat.enrichment, mpra=="mpra")
    PICS_threshold_emvars_loci_plot2 <-  ggplot(dat.enrichment2, aes( y=fold, x=pics)) + 
    geom_bar(position="dodge", stat="identity", color="black",aes(fill=-log10(p))) +  scale_fill_gradient(low= "lightgray",high="#E14A4A",limits = c(0, max(-log10(dat.enrichment$p)))) +
    facet_grid(~mpra) +
    theme_bw()+ 
    geom_hline(yintercept=1, linetype="dashed", color = "black")+
    ...
    geom_text(aes(label=round(-log10(p),2)), position="dodge", vjust=-0.75) +
    ggtitle("emVar enrichment and DHS enrichment for PICS probable causal variants (emVars loci only plot)", subtitle = subtitle) 
  PICS_threshold_emvars_loci_plot2
  
  
  # Do the same for the second plot
plots_name3 <- paste0(plots_id, "_", "Simple_PICS_threshold_emvars_loci",".pdf")
ggsave(plots_name3,plot = PICS_threshold_emvars_loci_plot2, path = plots.dir, width = 10, height = 5) 
```
...
# Calculate p-value and enrichment values
  ...
  dat.enrichment[i,]$p<-(p.adjust(fisher.test(rbind(c(a,b), c(c, d)), alternative="two.sided")$p.value, method = "bonferroni", n = 8)) #Calculate p-value. 
  dat.enrichment[i,]$odds<-fisher.test(rbind(c(a,b), c(c, d)))$estimate #Calculate odds ratio
  dat.enrichment[i,]$lower.conf<-fisher.test(rbind(c(a,b), c(c, d)))$conf.int[1] #Calculate lower confidence interval
  dat.enrichment[i,]$upper.conf<-fisher.test(rbind(c(a,b), c(c, d)))$conf.int[2] #Calculate higher confidence interval
  }
...
# Format chromhmm
  ...
  dat.enrichment[i,]$p<-(p.adjust(fisher.test(rbind(c(a,b), c(c, d)), alternative="two.sided")$p.value, method = "bonferroni", n = 36)) #calculate p-value. This P-value is BF corrected.
  dat.enrichment[i,]$odds<-fisher.test(rbind(c(a,b), c(c, d)))$estimate #Calculate odds ratio
  dat.enrichment[i,]$lower.conf<-fisher.test(rbind(c(a,b), c(c, d)))$conf.int[1] #Calculate lower confidence interval
  dat.enrichment[i,]$upper.conf<-fisher.test(rbind(c(a,b), c(c, d)))$conf.int[2] #Calculate higher confidence interval
  }
