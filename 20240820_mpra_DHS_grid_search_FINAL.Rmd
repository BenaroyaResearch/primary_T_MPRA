...
dat.plot<-data.frame(sample= sample.dat$sample, pcre_fold=0, pcre_p=0, a=0, stringsAsFactors = F)
...
TP_sample_size <- dat.plot$a
   ...
   tcell.precision.skew.dat <- rbind(tcell.precision.skew.dat, data.frame(k, j, pcre_p,pcre_enrichment,TP_sample_size)) # makes the data frame used for the heat map
  }
}

names(tcell.precision.skew.dat) <- c("FC", "pAdj", "pcre_p", "pcre_fold","TP_sample_size")
```

Save the T-cell precision skew data. Unhash to save. 
```{r}
# write.table(tcell.precision.skew.dat,paste0(dhs.data.dir,"tcell.precision.skew.dat.txt"), row.names=F, col.names=T, sep="\t", quote=F)
```
...
TP_sample_size <- dat.plot$a
   ...
   unstim.jurkat.precision.skew.dat <- rbind(unstim.jurkat.precision.skew.dat, data.frame(k, j, pcre_p,pcre_enrichment,TP_sample_size)) # makes the data frame used for the heat map
  }
}

names(unstim.jurkat.precision.skew.dat) <- c("FC", "pAdj", "pcre_p", "pcre_fold","TP_sample_size")
```

Save the jurkat precision skew data. Unhash to save. 
```{r, eval=FALSE}
# write.table(unstim.jurkat.precision.skew.dat,grid.search.data.dir,"unstim.jurkat.precision.skew.dat.txt", row.names=F, col.names=T, sep="\t", quote=F)
```
